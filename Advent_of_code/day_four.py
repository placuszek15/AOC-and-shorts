...
def find_correct_passports_1(puzzle_input):
	control_group = ["byr","iyr","eyr","hgt","hcl","ecl","pid"]

	return [i for i in 
	[piece.replace("\n","!!!").replace(":","!!!").replace(" ", "!!!").split("!!!")
	for piece in puzzle_input] if len([c_str for c_str in control_group if c_str in i]) == len(control_group)]

def find_correct_passports_2(correct_p_list):
	correct = 0
	safe_values = {
	"byr": lambda a: int(a) in range(1920,2003), 
	"iyr":lambda a: int(a) in range(2010,2021), 
	"eyr": lambda a: int(a) in range(2020,2031) ,
	"hgt":lambda a: bool(re.match("1[5-8][0-9]cm|1[9][0-3]cm|59in|6[0-9]in|7[0-6]in",a)) ,
	"hcl": lambda a: bool(re.match("^#[0-9a-f]{6}$",a)), 
	"ecl":lambda a: a in ["amb","blu","brn","gry","grn","hzl","oth"] ,
	"pid": lambda a: bool(re.match("^[0-9]{9}$",a)), 
	"cid": lambda a: True
	}

	for passport in correct_p_list:
		d = {k:v for k,v in sorted(zip(passport[::2], passport[1::2]))}
		if all((b := [safe_values[k](d[k]) for k in d.keys()])):
			correct += 1
	return correct

a = find_correct_passports_1(puzzle_input_list)
print(find_correct_passports_2(find_correct_passports_1(puzzle_input_list)))
